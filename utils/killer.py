...
class Killer:
    ...
    def __loginWithCookies(self):
        ...
        try:
            ...
            print(f"本地缓存验证异常（网络错误）: {e}", flush=True)
            # 网络错误时不要跳到Playwright，直接传播错误
    ...
    def __loginWithPlaywright(self):
        ...
        try:
            ...
            return (False, self.LOGIN_ERR_AUTH)
        ...
        try:
            ...
            print(f"登录异常: {e}")
            return (False, self.LOGIN_ERR_NETWORK)

        if not success or not cookies:
            if err and ("CONNECTION_RESET" in str(err) or "CONNECTION_REFUSED" in str(err) or "ERR_NAME" in str(err) or "timeout" in str(err).lower()):
                return (False, self.LOGIN_ERR_NETWORK)
            return (False, self.LOGIN_ERR_AUTH)
    ...
    def login(self):
        ...
        if self.__loginWithCookies():
            return (True, None)
        # 如果cookie验证时就是网络错误，直接标记为网络错误跳过Playwright
